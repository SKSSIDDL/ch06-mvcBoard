...
public class ModifyAction implements Action{
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		BoardVO db_board = dao.getBoard(boardVO.getNum()); //비밀번호 인증을 위해 읽어옴
		boolean check = false;
		if(db_board!=null) {//1건의 레코드 有
			//비밀번호 일치 여부 체크
			check = db_board.isCheckPassword(boardVO.getPasswd());
		}
		if(check) { //비밀번호 인증 성공
			dao.update(boardVO);
		...
		request.setAttribute("check", check); //UI처리를 위해 check가 true인지 false인지 작성
